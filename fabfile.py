...
def set_environment_variables(env_prefix):
    ...
    heroku_app = "{0}-{1}".format(os.environ["HEROKU_PREFIX"], env_prefix)
    local(
        f"heroku config:set DJANGO_SETTINGS_MODULE=config.settings.{which_settings} --app {heroku_app}"
    )
    ...
    local(
        "heroku config:set {}={} --app {}".format(
            "DJANGO_AWS_STORAGE_BUCKET_NAME", bucket_name, heroku_app
        )
    )
...
def raw_update_app(env_prefix="uat", branch="master"):
    ...
    if False:  # Using expensive bigger workers
        ...
        local(
            f"heroku ps:resize web=standard-1x -a {heroku_app}"
        )  # Resize web to be compatible with performance workers
...
def build_app(env_prefix="uat"):
    ...
    try:
        ...
        if env_prefix != "prod":
            ...
            raise Exception(
                "Must stop if an error when deleting a production database."
            )
